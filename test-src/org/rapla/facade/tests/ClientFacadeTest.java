...
public class ClientFacadeTest extends RaplaTestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        facade = (ClientFacade) getContext().lookup(ClientFacade.ROLE );
    ...
    public void testConflicts() throws Exception {
        //
    	List<Allocatable> allocatables = Arrays.asList(facade.getAllocatables());
        Conflict[] conflicts= facade.getConflicts( allocatables,(Date)null);
        ...
        conflicts = facade.getConflicts( allocatables,(Date) null );
        assertEquals( 1, conflicts.length );
        HashSet<Conflict> set = new HashSet<Conflict>( Arrays.asList( conflicts ));
        Conflict[] conflictsAfter = facade.getConflicts(allocatables, (Date)null );
        assertEquals( conflicts[0], conflictsAfter[0] );

        assertTrue ( set.containsAll( new HashSet<Conflict>( Arrays.asList( conflictsAfter ))));
    ...
    public void testPreferenceDependencies() throws RaplaException {
        ...
        RaplaMap<Allocatable> selected = facade.newRaplaMap( Collections.singleton( allocatable));
        CalendarModelConfiguration config = facade.newRaplaCalendarModel( selected, null,null, "", null, null, null, WeekViewFactory.WEEK_VIEW, null);
