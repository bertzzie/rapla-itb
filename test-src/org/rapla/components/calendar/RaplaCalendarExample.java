    ...
    public RaplaCalendarExample()
    {
        ...
        listener = new DateChangeListener()
        {
            ...
            public void dateChanged( DateChangeEvent evt )
            {
                ...
                Date date = ( (RaplaCalendar) raplaCalendars.get( index ) ).getDate();
                Date time = ( (RaplaTime) raplaTimes.get( index ) ).getTime();
                TimeZone timeZone = ( (RaplaCalendar) raplaCalendars.get( index ) ).getTimeZone();
                ...
                try
                {
                    for ( int i = 0; i < raplaCalendars.size(); i++ )
                    {
                        if ( i == index )
                            continue;
                        ( (RaplaCalendar) raplaCalendars.get( i ) ).setDate( dateTime );
                        ( (RaplaTime) raplaTimes.get( i ) ).setTime( dateTime );
                    }
                }
                finally
                {
        ...
        testCalendar.setDateRenderer( new WeekendHighlightRenderer()
        {
            public Color getBackgroundColor( int dayOfWeek, int day, int month, int year )
            {
                if ( day == 3 && month == Calendar.OCTOBER )
                    return Color.green;
                return super.getBackgroundColor( dayOfWeek, day, month, year );
            }
            ...
            public String getToolTipText( int dayOfWeek, int day, int month, int year )
            {
                ...
                return super.getToolTipText( dayOfWeek, day, month, year );
            }
        } );
        testCalendar.addDateChangeListener( listener );
