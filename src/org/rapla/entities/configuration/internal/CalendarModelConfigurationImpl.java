...
public CalendarModelConfigurationImpl( RaplaMap<? extends RaplaObject> selected, ClassificationFilter[] filter,   
   boolean defaultResourceTypes, boolean defaultEventTypes,String title, Date startDate, Date endDate, Date selectedDate,String view,RaplaMap<String> extensionMap) {
       this.view = view;
   ...
   public void resolveEntities( EntityResolver resolver) throws EntityNotFoundException {
    ...
    public Iterator<RefEntity<?>> getReferences() {
        RaplaMapImpl<RaplaObject> map = (RaplaMapImpl<RaplaObject>)selected;
        Iterator<RefEntity<?>> references = super.getReferences();
		return new IteratorChain<RefEntity<?>>(references, map.getReferences());
