...
{ 
    ...
    private void switchRepeatings() {
        if (noRepeating.isSelected()) {
            ...
            RepeatingType repeatingType = getCurrentRepeatingType();
            int repeatings = getCalendarOptions().isInfiniteRepeating() ? -1 : (1 * getCalendarOptions().getnTimes());  //BJO 00000012 -1:infinite; >0:=n-times
    ...
    */
    ...
    private void setToWholeDays(final boolean oneDayEvent) {
        ...
        if (wasSet && !oneDayEvent)
        {
            ...
            int startHour = calenderOptions.getWorktimeStart();
            int endHour  = calenderOptions.getWorktimeEnd();
            Date start = new Date( appointment.getStart().getTime() + startHour * DateTools.MILLISECONDS_PER_HOUR);
            Date end = new Date( appointment.getEnd().getTime() - DateTools.MILLISECONDS_PER_DAY + endHour * DateTools.MILLISECONDS_PER_HOUR);
    ...
    class SingleEditor implements DateChangeListener, Disposable {
        ...
        public SingleEditor() {
            ...
            double size[][] =
                {{pre, 5, pre, 10, pre,5,pre,5,pre},  // Columns
        ...
        private void processChange(final Object source) {
            ...
            try {
                ...
                if (source == startDate || source == startTime) {
                    ...
                    endDate.setDate(DateTools.addDays(newEnd,oneDayEventCheckBox.isSelected() ? -1:0));
                ...
                if (source == endDate) {
                    Date newEnd = raplaLocale.toDate(DateTools.addDays(endDate.getDate(),oneDayEventCheckBox.isSelected() ? -1:0),endTime.getTime());
                    if (appointment.getStart().after(newEnd)) {
                ...
                if (source == oneDayEventCheckBox) {
                    ...
                    endDate.setDate(DateTools.addDays(appointment.getEnd(),oneDayEventCheckBox.isSelected() ? -1:0));
        ...
        private void mapFromAppointment() {
            ...
            try {
                ...
                endDate.setDate(DateTools.addDays(appointment.getEnd(),wholeDaysSet ? -1:0));
                startTime.setTime(appointment.getStart());
                endTime.setTime(appointment.getEnd());
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            try {
				...
				if (evt.getSource() == convertButton)
				{
					...
					ActionListener[] listeners = (ActionListener[]) listenerList.toArray(new ActionListener[] {});
					for (int i = 0; i < listeners.length; i++)
					{
                ...
                if (evt.getSource() == endingChooser) {
                	// repeating has changed to UNTIL, the default endDate will be set  
                    int index = endingChooser.getSelectedIndex();
                    if (index == REPEAT_UNTIL) {                
                    	Date slotDate = appointment.getReservation().getSelectedSlotDate();
                    	if (slotDate != null)
    ...
    class ExceptionEditor implements ActionListener,ListSelectionListener {
        ...
        public void mapFromAppointment() {
            ...
            Date exceptDate = appointment.getReservation().getSelectedSlotDate();
            if( exceptDate == null)
            	exceptionDate.setDate( appointment.getStart());
            else
        ...
        private void removeException() {
            ...
            for ( int i = 0; i< selectedExceptions.length;i++) {
            	appointment.getRepeating().removeException((Date)selectedExceptions[i]);
            }
            specialExceptions.setListData(appointment.getRepeating().getExceptions());
            fireAppointmentChanged();
        }
