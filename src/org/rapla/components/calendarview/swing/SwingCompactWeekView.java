...
public class SwingCompactWeekView extends AbstractSwingCalendar
{
    ...
    public void rebuild() {
        ...
        for (int row=0;row<rowsize;row++) {
            ...
            for (int column=0;column < columns; column++) {
                ...
                SmallDaySlot field = createField(  column );
    ...
    private SmallDaySlot createField( int column)  {
...
//    			if ( DateTools.cutDate( block.getStart()).equals( startDate)) {
	...
	public void updateSize(int width) {
		...
		int newWidth = (width - leftColumnSize -20 - realColumns) /  (Math.max(1,realColumns-1));
