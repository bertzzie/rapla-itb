...
public abstract class AbstractGroupStrategy implements BuildStrategy {
	...
	protected void insertDay(CalendarView wv, int column,List<Block> blockList) {
        ...
        while (it.hasNext()) {
            ...
            for (int i=0;i<slot.size();i++) {
                wv.addBlock((Block)slot.get(i),column,slotCount);
            }
            slotCount ++;
        }
    }
    ...
    protected List<List<Block>> getSortedSlots(List<Block> blockList) {
        ...
        List<List<Block>> slots = new ArrayList<List<Block>>(group(blockList));
...
//        {
//            // 0 zero appointments will cause a collision
//            if ( start1.equals( end1) || start2.equals( end2))
//            {
//                return true;
//            }
//            boolean result = start1.before(end2) && start2.before(end1);
//            return result;
//        }
//        return false;
    }
