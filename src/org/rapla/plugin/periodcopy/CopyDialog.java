...
class CopyDialog extends RaplaGUIComponent implements RaplaWidget
{
    ...
    public CopyDialog(RaplaContext sm) throws RaplaException {
        ...
        final PeriodImpl customDest = new PeriodImpl(null, null);
        customDest.setName(getString("custom_period"));
        customDest.setStart( getQuery().today());
        customDest.setEnd( null);
        ...
        sourcePeriodChooser.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				updateView();
				if ( sourcePeriodChooser.getSelectedIndex() > 0)
				{
    ...
    public boolean isSourceDifferentFromDest()
    {
    	Date sourceStart = getSourceStart();
    	Date destStart = getDestStart();
        if (sourceStart == destStart)
        {
        	return false;
        }
        if ( sourceStart == null || destStart == null)
        {
        	return true;
        }
        return sourceStart.equals( destStart);
    }
