...
public class CalendarListPageGenerator extends RaplaComponent implements RaplaPageGenerator
{
            ...
            throws IOException, ServletException
    {
        try
        {
			...
			String calendarName = getQuery().getPreferences( null ).getEntryAsString(RaplaStartOption.TITLE, getString("rapla.title"));
            out.println( "<html>" );
            out.println( "<head>" );
            ...
            for (User user : sortedUsers)
            {
                ...
                CalendarModelConfiguration defaultConf = (CalendarModelConfiguration) preferences.getEntry( CalendarModelConfiguration.CONFIG_ENTRY );
                if ( defaultConf != null)
                {
                    completeMap.put( getString("default"), defaultConf);
                }
                
                final RaplaMap<CalendarModelConfiguration> raplaMap =  preferences.getEntry( "org.rapla.plugin.autoexport" );
                if ( raplaMap != null)
                {
