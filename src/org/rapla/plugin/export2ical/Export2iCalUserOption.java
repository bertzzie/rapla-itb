...
public class Export2iCalUserOption extends RaplaGUIComponent implements OptionPanel, ActionListener {
    ...
    public Export2iCalUserOption(RaplaContext sm, Configuration config) throws RaplaException {
		super(sm);
	...
	public void createList() throws RaplaException {
        ...
        panel.add(new JLabel("participation status:"), "1,8");
	...
	public void show() throws RaplaException {
        ...
        user_export_attendees = preferences.getEntryAsBoolean(Export2iCalPlugin.EXPORT_ATTENDEES, global_export_attendees);
        user_export_attendees_participants_status = preferences.getEntryAsString(Export2iCalPlugin.EXPORT_ATTENDEES_PARTICIPATION_STATUS, global_export_attendees_participants_status);
	...
	public void commit() {
		...
		if(!chkUseUserdefinedIntervall.isSelected()){
			preferences.putEntry(Export2iCalPlugin.PREF_BEFORE_DAYS, (RaplaObject)null);
			preferences.putEntry(Export2iCalPlugin.PREF_AFTER_DAYS, (RaplaObject)null);
		}else{
			preferences.putEntry(Export2iCalPlugin.PREF_BEFORE_DAYS, this.spiDaysBefore.getValue().toString());
			preferences.putEntry(Export2iCalPlugin.PREF_AFTER_DAYS, this.spiDaysAfter.getValue().toString());
		}

        preferences.putEntry(Export2iCalPlugin.EXPORT_ATTENDEES, ""+chkExportAttendees.isSelected());
        preferences.putEntry(Export2iCalPlugin.EXPORT_ATTENDEES_PARTICIPATION_STATUS, ""+cbDefaultParticipationsStatusRessourceAttribute.getSelectedItem().toString());
