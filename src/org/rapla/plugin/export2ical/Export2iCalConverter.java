...
public class Export2iCalConverter extends RaplaComponent {
    ...
    private int worktimeStartHour;
    private int worktimeEndHour;
        ...
        if (config != null)
            {
                ...
                doExportAsMeeting = preferences == null ? global_export_attendees : preferences.getEntryAsBoolean(Export2iCalPlugin.EXPORT_ATTENDEES, global_export_attendees);
                exportAttendeesParticipationStatus = preferences == null ? global_export_attendees_participation_status : preferences.getEntryAsString(Export2iCalPlugin.EXPORT_ATTENDEES_PARTICIPATION_STATUS, global_export_attendees_participation_status);
                ...
                try {
                    ...
                    getLogger().info("ExportAttendeesMailAttribute is not set. So do not export as meeting", e);
                }
    ...
    private void initConfiguration() {
        try {
            ...
            config = raplaConfiguration.find("class", Export2iCalPlugin.PLUGIN_CLASS);
        } catch (RaplaException e) {
            getLogger().error("Cannot read plugin configuration");
        }
    }

    public void setiCalendarWorktime(CalendarOptions options, RaplaLocale locale) throws RaplaException {
        calendar = locale.createCalendar();
        worktimeStartHour = options.getWorktimeStart();
        worktimeEndHour = options.getWorktimeEnd();
    }
    ...
    private VEvent createVEvent(Appointment appointment) {
        ...
        boolean isAllDayEvent = appointment.isWholeDaysSet() || isAllDayEvent(appointment.getStart(), appointment.getEnd());
    ...
    private boolean isAllDayEvent(Date start, Date end) {
        ...
        boolean startsOnWorkTimeStart = startHour == worktimeStartHour;
        boolean endsOnWorkTimeEnd = endHour == worktimeEndHour;

        return startsOnWorkTimeStart && endsOnWorkTimeEnd;
    }

    /**
     * F�gt dem Termin das Modifizierungsdatum hinzu
     *
     * @param appointment
     * @param event
     */
    ...
    /**
     * F�gt dem Termin den DTSTAMP hinzu lt. rfc muss dies dem Erstellungdatum
    ...
    private void addEndDateToEvent(Appointment appointment, PropertyList properties, boolean isAllDayEvent) {
        ...
        if (isAllDayEvent) {
            ...
            if (appointment.getRepeating() == null) {
                ...
                DtEnd end = getEndDateProperty(appointment, endDate);
                properties.add(end);
            }
        }
    }

    private DtEnd getEndDateProperty(Appointment appointment, Date endDate) {
    ...
    private void addStartDateToEvent(Appointment appointment, PropertyList properties, boolean isAllDayEvent) {
        ...
        if (isAllDayEvent) {
            ...
            if (appointment.getRepeating() == null) {
                ...
                DtStart start = getStartDateProperty(appointment, startDate);
                properties.add(start);
            }
        }
    }

    private DtStart getStartDateProperty(Appointment appointment, Date startDate) {
    ...
    private DateTime convertRaplaLocaleToUTC(Date date) {
        return convertRaplaLocaleToUTC(date, timezone);
    }

    public static DateTime convertUtcToRaplaLocale(Date date, TimeZone timezone) {
    ...
    /**
     * Diese Methode wandelt das Rapla Datum in UTC um
     *
     * @param date Das zu konvertierende lokale Datum.
     * @return Das nach UTC konvertierte Datum.
     */
    public static DateTime convertRaplaLocaleToUTC(Date date, TimeZone timezone) {
